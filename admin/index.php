...
<body class=" theme-blue">
  ...
  <!--<![endif]-->
   ...
   <!-- Small boxes (Stat box) -->
	       ...
	       <input type='text' class="form-control" style="margin-bottom: 4px;" name='qcari' placeholder='Cari Berdasarkan Tgl Pemeriksaan ' required /> 
           <input type='submit' value='Cari Data' class="btn btn-sm btn-primary" /> <a href='index.php' class="btn btn-sm btn-success" >Refresh</i></a>
          	</div>
              </div>
		...
		<!-- Fungsi Menampikan Isi dari Database -->
						  <?php
									$query1="SELECT tb_pasien.id_pasien, tb_pasien.no_badge, tb_pasien.nama_pasien, tb_pasien.no_hp, tb_dokter.nama_dokter, tb_rekmed.tgl_rekmed	
									FROM tb_pasien, tb_dokter, tb_rekmed WHERE 
									tb_rekmed.id_pasien=tb_pasien.id_pasien AND tb_rekmed.id_dokter=tb_dokter.id_dokter
									ORDER BY id_rekmed DESC LIMIT 5";
									
									// Fungsi Search
							if(isset($_POST['qcari'])){
						   $qcari=$_POST['qcari'];
						   $query1="SELECT tb_pasien.no_badge, tb_pasien.nama_pasien, tb_pasien.no_hp, tb_dokter.nama_dokter, tb_rekmed.tgl_rekmed	
									FROM tb_pasien, tb_dokter, tb_rekmed WHERE 
									tb_rekmed.id_pasien=tb_pasien.id_pasien AND tb_rekmed.id_dokter=tb_dokter.id_dokter
...
<div class="row">
    <div class="col-sm-12 col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading no-collapse">Data Rekap Medis Pasien<span class="label label-warning">
			<!-- Fungsi Menampilkan Total Rekap Medis dari Database -->
	<b><?php $tampil=mysqli_query($koneksi, "select * from tb_rekmed order by id_rekmed desc");
		$total=mysqli_num_rows($tampil);
		?>
		<?php echo "$total"; ?>
		<!-- End --></span> </span></div>
            <table class="table table-bordered table-striped">
              <thead>
                <tr>
				  ...
				  <th><center>Dokter Pemeriksa</center></th>
