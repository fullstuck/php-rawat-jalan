  ...
  <!--<![endif]-->
	...
	<b><?php $tampil=mysqli_query($koneksi, "select * from tb_obat order by id_obat desc");
		...
		<!-- End --></span> Obat</p>
            ...
            <h1 class="page-title">Edit Data Rekap Medis</h1>
                    <ul class="breadcrumb">
            <li><a href="index.php">Home</a> </li>
            <li class="active">Edit Data Rekap Medis</li>
        </ul>
					...
					<?php
					$kd = $_GET['kd'];
					$sql = mysqli_query($koneksi, "SELECT * FROM tb_rekmed a JOIN tb_pasien b ON a.id_pasien=b.id_pasien JOIN 
					tb_dokter c ON a.id_dokter=c.id_dokter WHERE id_rekmed='$kd'");
					...
					if(isset($_POST['update'])){
							...
							$tgl_rekmed			= $_POST['tgl_rekmed'];
							$id_pasien			= $_POST['id_pasien'];
							$id_dokter			= $_POST['id_dokter'];
							$keterangan			= $_POST['keterangan'];
							$itemCount 			= count($_POST["nama_obat"]);
							$itemValues			=0;
							$Value = "";
							$queryValue = "";
							for($i=0;$i<$itemCount;$i++) {
								if(!empty($_POST["nama_obat"][$i])) {
									$itemValues++;
									if($Value!="") {
										$Value .= ", ";
									}
									$Value .= $_POST["nama_obat"][$i];
								}
							}
							if($itemValues!=0) {
								$update = mysqli_query($koneksi, "UPDATE tb_rekmed SET  tgl_rekmed='$tgl_rekmed', id_pasien='$id_pasien', id_dokter='$id_dokter', 
								nama_obat='$Value', keterangan='$keterangan' WHERE id_rekmed='$id_rekmed'") or die(mysqli_error());
								if($update){
...
<div class="row">
     ...
     <div class="col-md-12 col-sm-12">
                         ...
                         <div class="alert alert-default alert-gradient alert-dismissible fade in" role="alert">
                           <strong><h4> Edit Data Rekap Medis</h4></strong>
                        </div>
						  ...
						  <!-- Fungsi Form Tambah Data Kasir -->
                            <div class="panel-body">
                              <div class="form">
                                  <form class="form-horizontal style-form" action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
									...
									<div class="form-group ">
                                          <label for="id_rekmed" class="control-label col-lg-2">ID Rekap Medis </label>
                                          <div class="col-lg-2">
                                              <input class="form-control" id="id_rekmed" type="text" name="id_rekmed" value="<?php echo $row['id_rekmed'];?>" readonly="readonly" autofocus="on"/>
                                          </div>
                                      </div>
									  
									<div class="form-group ">
                                          <label for="tgl_rekmed" class="control-label col-lg-2">Tanggal Rekap Medis</label>
                                          <div class="col-lg-3">
                                              <input class="input-group date form-control" data-date="" data-date-format="yyyy-mm-dd" id="tgl_rekmed" type="text" name="tgl_rekmed" value="<?php echo $row['tgl_rekmed'];?>" required/>
                                          </div>
                                      </div>
								<div class="form-group">
                              ...
                              <?php
									...
									{
								?>
							<option value="<?php echo $data['id_pasien']; $id_pasien=$data['id_pasien']?>"><?php echo $data['nama_pasien'];?></option>
						    <?php } ?>
                              </select>
                              </div>
                          </div>
						   <div class="form-group">
                              ...
                              <?php
									...
									{
								?>
							<option value="<?php echo $data['id_dokter']; $id_dokter=$data['id_dokter']?>"><?php echo $data['nama_dokter'];?></option>
						    <?php } ?>
                              </select>
                              </div>
                          </div>
						   <div id="product">
                              <?php require_once("beri_obat.php") ?>
                          </div>
						  <div class="form-group ">
									 <div class="col-lg-offset-2 col-lg-10">
										<input type="button" name="add_item" value="Tambah Obat" onClick="addMore();" />
										<input type="button" name="del_item" value="Hapus Obat" onClick="deleteRow();" />
									</div>
							</div>
									...
									<div class="form-group ">
                                          <label for="keterangan" class="control-label col-lg-2">Keterangan </label>
                                              ...
                                              <textarea name="keterangan" id="keterangan" cols="60%" rows="4"><?php echo $row['keterangan'];?></textarea>
                                      ...
                                      <div class="form-group">
                                          <div class="col-lg-offset-2 col-lg-10">
                                              ...
                                              <a class="btn btn-default" type="button" href="rekap.php">Cancel</a>
                                          </div>
                                      </div>
                                  </form>
                              </div>
	...
	<script>
	function addMore() {
		$("<div>").load("beri_obat.php", function() {
