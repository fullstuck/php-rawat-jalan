	...
	<b><?php $tampil=mysqli_query($koneksi, "select * from tb_obat order by id_obat desc");
		...
		<?php
             if(isset($_GET['hal']) == 'hapus'){
				$id_rekmed = $_GET['kd'];
				$cek = mysqli_query($koneksi, "SELECT * FROM tb_rekmed WHERE id_rekmed='$id_rekmed'");
				if(mysqli_num_rows($cek) == 0){
					echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Data tidak ditemukan.</div>';
				}else{
					$delete = mysqli_query($koneksi, "DELETE FROM tb_rekmed WHERE id_rekmed='$id_rekmed'");
					if($delete){
            ...
            <h1 class="page-title">Rekap Medis</h1>
                    <ul class="breadcrumb">
            <li><a href="index.php">Home</a> </li>
            <li class="active">Rekap Medis</li>
        </ul>
...
<!-- Small boxes (Stat box) -->
	       ...
	       <input type='text' class="form-control" style="margin-bottom: 4px;" name='qcari' placeholder='Cari Berdasarkan Tgl Rekmed ' required /> 
           <input type='submit' value='Cari Data' class="btn btn-sm btn-primary" /> <a href='rekap.php' class="btn btn-sm btn-success" >Refresh</i></a>
          	</div>
              </div>
					...
					<?php
                    $query1="SELECT id_rekmed, tgl_rekmed, tb_pasien.nama_pasien, tb_dokter.nama_dokter, nama_obat, keterangan
							FROM tb_rekmed, tb_dokter, tb_pasien
							WHERE tb_rekmed.id_pasien=tb_pasien.id_pasien AND tb_rekmed.id_dokter=tb_dokter.id_dokter";
	               ...
	               $query1="SELECT id_rekmed, tgl_rekmed, tb_pasien.nama_pasien, tb_dokter.nama_dokter, nama_obat, keterangan
							FROM tb_rekmed, tb_dokter, tb_pasien
							WHERE tb_rekmed.id_pasien=tb_pasien.id_pasien AND tb_rekmed.id_dokter=tb_dokter.id_dokter
							AND tgl_rekmed like '%$qcari%'";
                    }
                    $tampil=mysqli_query($koneksi, $query1) or die(mysqli_error());
                    ?>	
						  <!-- End -->

							<p><a class="btn btn-primary icon-btn" href="tambah-rekap.php"><i class="fa fa-plus"></i> Tambah Rekap Medis	</a></p>

<div class="row">
    <div class="col-sm-12 col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading no-collapse">Data Rekap Medis Pasien</div>
            <table class="table table-bordered table-striped">
              <thead>
                <tr>
				  ...
				  <th><center>Tgl. Pemeriksaan</center></th>
			  ...
			  <!-- Fungsi Menampilkan data dari database -->	
                    ...
                    { $no++; ?>
								...
								<td><center><a href="detail_rekap.php?hal=edit&kd=<?php echo $data['id_rekmed'];?>"><span class="fa fa-user fa-lg"></span> <?php echo $data['nama_pasien'];?></span></td>
								<td><center><?php echo $data['nama_dokter'];?></center></td>
								<td><center><?php echo $data['nama_obat'];?></center></td>
								<td><center><span class="label label-danger"><?php echo $data['tgl_rekmed']; ?></span></center></td>
								
								<td><center><div id="thanks"><a class="btn btn-sm btn-info" data-placement="bottom" data-toggle="tooltip" title="Edit Rekap" href="edit-rekap.php?hal=edit&kd=<?php echo $data['id_rekmed'];?>"><span class="fa fa-pencil"> Edit</span></a>  
                        <a onclick="return confirm ('Yakin hapus <?php echo $data['tgl_rekmed'];?>.?');" class="btn btn-sm btn-danger tooltips" data-placement="bottom" data-toggle="tooltip" title="Hapus Rekap" href="hapus-rekap.php?hal=hapus&kd=<?php echo $data['id_rekmed'];?>"><span class="fa fa-trash-o"> Delete</span></a></center></td></tr></div>
                 <?php   
              } 
              ?>   
            <!-- End -->  
